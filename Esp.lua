-- [[ KOPI'S ESP - PART 1 OF 2 - PASTE THIS FIRST ]]
...
-- [[ END OF PART 1 - NOW PASTE PART 2 DIRECTLY BELOW ]]
-- [[ KOPI'S ESP - PART 2 OF 2 - PASTE THIS AFTER PART 1 ]]
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= LocalPlayer and p.Character then
			...
			if hum and hrp and hum.Health > 0 then
				...
				-- WALLCHECK: Check visibility
				local isVisible = IsVisible(hrp)
				local wallAlpha = isVisible and 1 or 0.3
				
				if not ESPStore[p] then
					ESPStore[p] = {
						Box = D("Square", {Thickness=1.5, Filled=false, Transparency=0}),
						BoxOutline = D("Square", {Thickness=3, Filled=false, Transparency=0, Color=Color3.new(0,0,0)}),
						Tracer = D("Line", {Thickness=1, Transparency=0}),
				...
				-- UPDATE CHAMS COLOR/VISIBILITY WITH WALLCHECK
				...
				if not cham then
					...
					cham.OutlineTransparency = isVisible and 0.2 or 0.7
				end
				...
				if onScreen then
					...
					if ESP_SETTINGS.Box then
						...
						esp.Box.Color = col
						esp.Box.Transparency = 1 - wallAlpha
						esp.BoxOutline.Size = Vector2.new(w, h)
						esp.BoxOutline.Position = esp.Box.Position
						esp.BoxOutline.Transparency = 1 - (wallAlpha * 0.5)
					end
					...
					if ESP_SETTINGS.Tracers then
						...
						esp.Tracer.Transparency = 1 - wallAlpha
					end
					...
					if ESP_SETTINGS.Names then
						...
						esp.Name.Color = col
						esp.Name.Transparency = 1 - wallAlpha
					end
					...
					if esp.Info.Visible then
						...
						esp.Info.Color = col 
						esp.Info.Transparency = 1 - wallAlpha
					end
					...
					if doSkel then
						...
						if hObj then
							...
							if hon then
								...
								esp.Head.Color = col
								esp.Head.Transparency = 1 - wallAlpha
							end
						end
